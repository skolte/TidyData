...
run_analysis <- function()
{
        ...
        tryCatch({
                ...
                message("Error : The directory with 'run_analysis.R' script cannot access 'UCI HAR DATASET'. Please ensure that the 'run_analysis.R' script is in the folder where 'UCI HAR DATASET' is located. It may be necessary to set working directory to the script location in R Studio / Session / Set Working Directory / To Source File Location.")
                errorOccurred <- TRUE
        }, finally = {
        ...
        if(!errorOccurred)
        {
                ...
                myTidyData <- grouped %>% summarise_each(funs(mean)) 
                ...
                oldNames <- colnames(myTidyData)
                newNames <- getReadableColumnNames()
                setnames(myTidyData, oldNames, newNames)
                write.table(myTidyData, "tidydata.txt", row.names=FALSE)  
                print("Created 'tidydata.txt' successfully.")
        }
}
